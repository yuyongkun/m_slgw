       <template>

		<!--主体-->
		<div class="container">

			<!--关于-->
			<div class="sec guanyu">
				<h1>关于影视动画</h1>
				<h2>以光之影、动之效，突破物理形象在影视中的</br>想象</h2>
				<p>影视动画是以视觉为载体的传播媒体，是向观众传递信息的最生动形象的方式，分为实拍电影与三维动画，或是两者的结合体。应用范围十分广泛，常用于不同行业的产品、营销、活动、品牌等宣传与传播上，是最受欢迎的传播手段之一。影视动画是利用计算机语言所创作出来的视觉体验。</p>
			</div>

			<!--案例-->

			<div class="sec anli">
				<div class="anli-title">
					<img src="../../../assets/images/duomeiti/yingshidonghua/wuxing.png" class="xing1">
					<h1>案例精选</h1>
					<img src="../../../assets/images/duomeiti/yingshidonghua/wuxing.png">
					<div class="cle"></div>
				</div>
				<p>非凡的影像画面，触碰心灵的视觉盛宴</p>
				<router-link to="/yingshihd/case/1.html">
				<img src="../../../assets/images/duomeiti/yingshidonghua/img1.jpg" class="anli-pic">
				<div class="anli-sum">
					<div class="anli-wenzi">
						<h1>汉京中心宣传片</h1>
						<p>汉京集团</p>
					</div>
					<img class="anli-arrow" src="../../../assets/images/duomeiti/yingshidonghua/arrow.jpg">
					<div class="cle"></div>
				</div>	
			</router-link>

				<router-link to="/yingshihd/case/2.html">
				<img src="../../../assets/images/duomeiti/yingshidonghua/img2.jpg" class="anli-pic">
				<div class="anli-sum">
					<div class="anli-wenzi">
						<h1>真理永恒</h1>
						<p>新华社</p>
					</div>
					<img class="anli-arrow" src="../../../assets/images/duomeiti/yingshidonghua/arrow.jpg">
					<div class="cle"></div>
				</div>
			</router-link>


				<router-link to="/yingshihd/case/3.html">
				<img src="../../../assets/images/duomeiti/yingshidonghua/img3.jpg" class="anli-pic">
				<div class="anli-sum">
					<div class="anli-wenzi">
						<h1>上合之合</h1>
						<p>新华社</p>
					</div>
					<img class="anli-arrow" src="../../../assets/images/duomeiti/yingshidonghua/arrow.jpg">
					<div class="cle"></div>
				</div>
				</router-link>

				<router-link to="/yingshihd/case/4.html">
				<img src="../../../assets/images/duomeiti/yingshidonghua/img4.jpg" class="anli-pic">
				<div class="anli-sum">
					<div class="anli-wenzi">
						<h1>保利·堂悦品牌宣传片</h1>
						<p>保利发展控股集团股份有限公司</p>
					</div>
					<img class="anli-arrow" src="../../../assets/images/duomeiti/yingshidonghua/arrow.jpg">
					<div class="cle"></div>
				</div>
				</router-link>

				<router-link to="/yingshihd/case/5.html">
				<img src="../../../assets/images/duomeiti/yingshidonghua/img5.jpg" class="anli-pic">
				<div class="anli-sum">
					<div class="anli-wenzi">
						<h1>南方航空安全须知宣传影片</h1>
						<p>南方航空</p>
					</div>
					<img class="anli-arrow" src="../../../assets/images/duomeiti/yingshidonghua/arrow.jpg">
					<div class="cle"></div>
				</div>
				</router-link>

				<router-link to="/yingshihd/case/6.html">
				<img src="../../../assets/images/duomeiti/yingshidonghua/img6.jpg" class="anli-pic">
				<div class="anli-sum">
					<div class="anli-wenzi">
						<h1>港珠澳大桥设计方案三维动画影片</h1>
						<p>客户名称暂未公开</p>
					</div>
					<img class="anli-arrow" src="../../../assets/images/duomeiti/yingshidonghua/arrow.jpg">
					<div class="cle"></div>
				</div>
				</router-link>

				<div class="anli-yin">
					<router-link to="/yingshihd/case/7.html">
					<img src="../../../assets/images/duomeiti/yingshidonghua/img7.jpg" class="anli-pic">
					<div class="anli-sum">
						<div class="anli-wenzi">
							<h1>乾隆南巡图第十二卷：回鸾</h1>
							<p>中国国家博物馆</p>
						</div>
						<img class="anli-arrow" src="../../../assets/images/duomeiti/yingshidonghua/arrow.jpg">
						<div class="cle"></div>
					</div>
					</router-link>

					<router-link to="/yingshihd/case/8.html">
					<img src="../../../assets/images/duomeiti/yingshidonghua/img8.jpg" class="anli-pic">
					<div class="anli-sum">
						<div class="anli-wenzi">
							<h1>建发家居「木竟」紫檀宣传片</h1>
							<p>厦门建发集团有限公司</p>
						</div>
						<img class="anli-arrow" src="../../../assets/images/duomeiti/yingshidonghua/arrow.jpg">
						<div class="cle"></div>
					</div>
				</router-link>

					<router-link to="/yingshihd/case/9.html">
					<img src="../../../assets/images/duomeiti/yingshidonghua/img9.jpg" class="anli-pic">
					<div class="anli-sum">
						<div class="anli-wenzi">
							<h1>清远长隆国际森林度假区规划宣传片</h1>
							<p>清远长隆投资有限公司</p>
						</div>
						<img class="anli-arrow" src="../../../assets/images/duomeiti/yingshidonghua/arrow.jpg">
						<div class="cle"></div>
					</div>
				</router-link>

					<router-link to="/yingshihd/case/10.html">
					<img src="../../../assets/images/duomeiti/yingshidonghua/img10.jpg" class="anli-pic">
					<div class="anli-sum">
						<div class="anli-wenzi">
							<h1>深圳湾湾汇招商片</h1>
							<p>深圳市地铁集团有限公司</p>
						</div>
						<img class="anli-arrow" src="../../../assets/images/duomeiti/yingshidonghua/arrow.jpg">
						<div class="cle"></div>
					</div>
				</router-link>

					<div class="btn btn-shouqi">收起更多案例</div>
				</div>
				<div class="btn btn-jiazai">加载更多案例</div>
			</div>

<site-map></site-map>
</div>
       </template>



	<script>
export default {
  mounted() {
    $(".anli-yin").hide();
    $(".btn-shouqi").hide();
    $(".btn-jiazai").click(function() {
      $(".anli-yin").show();
$(".btn-shouqi").show();
      $(".btn-jiazai").hide();
    });
    $(".btn-shouqi").click(function() {
      $(".anli-yin").hide();
      $(".btn-shouqi").hide();
      $(".btn-jiazai").show();
    });
  }
}

</script>

			